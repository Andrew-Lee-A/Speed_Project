//Creating bibtex file for a speed dealer dealing speed  through articles
...
function BibtexDisplay() {
    ...
    this.createTemplate = function(entry, output) {
        ...
        var orRegExp = new RegExp('\\s*[\\|\\|]\\s*', "gi"); 
        var eqRegExp = new RegExp('\\s*==\\s*', "gi"); 
...
function generateList(object, bibtexField) {
    ...
    if (bibtexField == "author") {
        ...
        for (var key in map) {
            ...
            displayTuples.push([lastName.toLowerCase(), 
                key, 
                map[key][1], 
                map[key][0]
            ]); 
